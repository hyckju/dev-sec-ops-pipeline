# Phase 2 — CI/CD 인터페이스 (작업 일지)

작성: 2026-06-01
관련 문서: [`progress-and-roadmap.md`](./progress-and-roadmap.md), [`phase-1-test-foundation.md`](./phase-1-test-foundation.md)

---

## 목적 (왜 이 작업을 했는가)

Phase 1에서 만든 API 계약 테스트(83 passed + 8 skipped)를 안전망 삼아, 백엔드를 **외부 CI/CD(GitHub Actions)에서 실제로 호출 가능한 형태**로 다듬었다. Phase 3(워크플로 작성)가 의존할 인터페이스 — 인증, 컨테이너 이미지, 가벼운 폴링 엔드포인트, 집계 필드 — 를 먼저 갖추는 것이 목표.

이번 세션 범위: **2.1~2.4 코드 작업만**. 신규 코드 테스트와 `SecurityScanException` 정리는 의도적으로 분리(다음 세션). 2.5 배포 위치는 의사결정 항목으로 코드 아님.

---

## 결과 한눈에

- 4개 작업(2.1~2.4) 구현 완료 + **신규 코드 테스트 11개 추가**, **회귀 0** — pytest **94 passed + 8 skipped**
...
우선순위 순. ①②는 Phase 2 마무리, ③부터 Phase 3.

### ① 신규 코드 테스트 작성 ✅ 완료 (2026-06-01)

Phase 1 안전망을 신규 코드까지 확장. 11건 추가, 전체 94 passed + 8 skipped (회귀 0).
...
- **2.5 백엔드 배포 위치 결정** ✅ (2026-06-01) — **이미지 레지스트리: DockerHub** 사용 확정.
  - `backend/Dockerfile`(2.2)로 빌드한 이미지를 DockerHub에 publish.
  - **publish 방식**: GitHub Actions 자동화(push/tag 시 `docker build` → DockerHub push). → Phase 3 ③에서 워크플로로 작성.
...
- (선택) `SecurityScanException` 정리 — 보류 유지. Phase 3에서 차단 정책 다룰 때 함께 처리 고려

### ③ Phase 3 — GitHub Actions 워크플로 (status/summary 인터페이스 위에서 착수)

이번에 만든 `/status`(폴링)와 `summary`(코멘트용)가 여기서 처음 소비된다.

- `.github/workflows/docker-publish.yml` (이 리포) — **2.5 결정 구현**
...
- `tests/integration/github/test_action_contract.py` — Action이 의존하는 응답 필드 스키마 snapshot (Phase 1에서 보류했던 것)
...
| semgrep 바이너리 dev 의존성화 | Phase 1의 8 skipped 활성화 (룰팩 회귀 감지) |
| `.idea/` gitignore 정리 | Phase 1 일지 §4 — 브랜치 전환 차단 재발 방지 |
