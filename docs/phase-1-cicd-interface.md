...
- 4개 작업(1.1~1.4) 구현 완료, **회귀 0** — pytest 여전히 83 passed + 8 skipped (2.26s)
...
### ① 신규 코드 테스트 작성 (다음 세션 1순위)

이번 세션에서 코드만 작성하고 분리한 항목. Phase 0 안전망을 신규 코드까지 확장한다.

- `tests/unit/api/test_deps_auth.py` — `verify_api_key` 단위 테스트
  - `API_KEY` 미설정 → 헤더 없어도 통과
  - `API_KEY` 설정 + 헤더 누락/불일치 → 401
  - `API_KEY` 설정 + 헤더 일치 → 통과
- `tests/integration/pipeline/test_pipelines_api.py` 보강 (기존 파일에 추가)
  - `GET /{id}/status` — 404, 200 응답 스키마(`current_step`/`completed_steps`/`vulnerability_count`), vulnerabilities 미포함 확인
  - `GET /{id}` `summary` 필드 — severity 카운트 합 == vulnerabilities 길이, `kev_count` 주입
  - 인증 활성(`monkeypatch settings.API_KEY`) 시 401, 헤더 통과 — **기존 13개 테스트가 키 없이 도므로 별도 격리 필요**
